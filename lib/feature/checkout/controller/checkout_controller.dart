import 'dart:convert';
import 'package:seohost/common/widgets/confirmation_widget.dart';
import 'package:seohost/feature/checkout/model/payment_response_model.dart';
...
class CheckOutController extends GetxController implements GetxService {
  ...
  final TextEditingController bookingNoteController = TextEditingController();
  ...
  Future<void> placeBookingRequest({
    ...
    try {
      if (Get.find<CartController>().cartList.isNotEmpty) {
        Response response = await checkoutRepo
            .placeBookingRequest(
              ...
              serviceLocation: Get.find<LocationController>()
                  .selectedServiceLocationType
                  .name,
              bookingNote: bookingNoteController.text.trim(),
            )
            .timeout(
        ...
        if (isBookingPlaced) {
          ...
          if (response.statusCode == 1 && paymentMethod != "offline_payment") {
            _isPlacedOrderSuccessfully = true;
            _bookingReadableId = "";
            updateState(PageState.complete);
            Get.offNamed(
            ...
            customSnackBar(
              'your_booking_has_been_placed_successfully'.tr,
              type: ToasterMessageType.success,
              margin: 55,
            );
            return;
          ...
          if (response.statusCode == 200) {
            customSnackBar(
              response.body?['message']?.toString() ?? 'فشل تأكيد الحجز',
            );
          }
        }
      } else {
      ...
      customSnackBar('حدث خطأ غير متوقع');
    } finally {
      _isLoading = false;
      update();
    }
  ...
  void resetBookingNote({bool shouldUpdate = false}) {
