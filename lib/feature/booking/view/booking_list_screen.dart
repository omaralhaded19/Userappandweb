...
import 'package:seohost/feature/booking/widget/booking_item_card.dart';
import 'package:seohost/feature/booking/widget/booking_status_tabs.dart';
...
class _BookingListScreenState extends State<BookingListScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    if (!ResponsiveHelper.isDesktop(context)) {
      return _buildQadhaBookingList(context, bookingScreenScrollController);
    }
        ...
        child: QadhaSoftScaffold(
          ...
          child: SafeArea(
            ...
            child: GetBuilder<ServiceBookingController>(
              builder: (serviceBookingController) {
                final bookingList = serviceBookingController.bookingList;
                return Column(
                  children: [
                    QadhaGradientHeader(
                      title: 'حجوزاتي',
                      ...
                      leading: widget.isFromMenu
                          ...
                          : null,
                      trailing: const FilterPopUpMenuWidget(),
                    ),
                    ...
                    SizedBox(
                      ...
                      child: ListView(
                        ...
                        children: BookingStatusTabs.values.map((status) {
                          final selected =
                              serviceBookingController.selectedBookingStatus ==
                              status;
                          return Padding(
                            padding: const EdgeInsetsDirectional.only(end: 10),
                            child: QadhaStatusPill(
                              text: status.name.tr,
                              icon: _statusIcon(status),
                              selected: selected,
                              onTap: () => serviceBookingController
                                  .updateBookingStatusTabs(status),
                            ),
                          );
                        }).toList(),
                      ),
                    ...
                    Expanded(
                      child: bookingList == null
                          ? const Center(child: BookingListItemShimmer())
                          : bookingList.isEmpty
                          ? NoDataScreen(
                              text: 'no_booking_request_available'.tr,
                              type: NoDataType.bookings,
  ...
  IconData _statusIcon(BookingStatusTabs status) {
    switch (status) {
      case BookingStatusTabs.pending:
        return Icons.hourglass_bottom_rounded;
      case BookingStatusTabs.accepted:
        return Icons.verified_rounded;
      case BookingStatusTabs.ongoing:
        return Icons.sync_rounded;
      case BookingStatusTabs.completed:
        return Icons.check_circle_rounded;
      case BookingStatusTabs.canceled:
        return Icons.cancel_rounded;
      case BookingStatusTabs.all:
