import 'dart:convert';
import 'package:seohost/feature/auth/view/update_profile_screen.dart';
import 'package:seohost/feature/booking/view/repeat_booking_details_screen.dart';
import 'package:seohost/feature/checkout/view/offline_payment_screen.dart';
import 'package:seohost/feature/provider/view/nearby_provider/near_by_provider_screen.dart';
import 'package:get/get.dart';
import 'package:seohost/utils/core_export.dart';

class RouteHelper {
  // ✅ Fix: safe param reader for web/app (prevents Illegal percent encoding in URI)
  static String _readJsonParam(String key) {
    final raw0 = Get.parameters[key];
    if (raw0 == null || raw0.isEmpty) return "";

    // In query strings, '+' is often used for spaces
    final raw = raw0.replaceAll('+', ' ');

    // Try decode -> jsonDecode (when still percent-encoded)
    try {
      return jsonDecode(Uri.decodeComponent(raw)) as String;
    } catch (_) {}

    // Try jsonDecode directly (when already decoded by GetX / platform)
    try {
  ...
  static const String categoryChildren = '/category-children';
  ...
  static String getCategoryChildrenRoute({
    required String categoryId,
    required String title,
    required int level,
  }) {
    final encodedTitle = Uri.encodeComponent(jsonEncode(title));
    return '$categoryChildren?id=$categoryId&title=$encodedTitle&level=$level';
  }
  ...
  static List<GetPage> routes = [
    ...
    // ✅ Fix here: use _readJsonParam to avoid Uri.decodeComponent crash on web
    GetPage(
      name: verification,
      page: () {
        String data = _readJsonParam('identity');
      ...
      page: () {
        return getRoute(
          CategoryChildrenScreen(
            parentId: Get.parameters['id'] ?? "",
            title: _readJsonParam('title'),
            level: int.tryParse(Get.parameters['level'] ?? '2') ?? 2,
          ),
        );
      },
    ),
    ...
    // ✅ Fix here: use _readJsonParam for update-profile params
    GetPage(
      name: updateProfile,
      page: () {
        String phone = _readJsonParam('phone');
        String email = _readJsonParam('email');
        String tempToken = _readJsonParam('temp-token');
        String userName = _readJsonParam('user-name');
